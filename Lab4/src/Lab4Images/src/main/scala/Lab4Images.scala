...
object Lab4Images {
  ...
  val TRAIN_IMAGE_PATH    = "data/flags"
  ...
  val TEST_INPUT_PATH     = "data/testflags"
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setAppName("Lab4Images").setMaster("local[*]")
    val sc=new SparkContext(sparkConf)

    //Load our images
    val flags = sc.wholeTextFiles(s"${TRAIN_IMAGE_PATH}/*/*.jpg")
    ...
    val testImages = sc.wholeTextFiles(s"${TEST_INPUT_PATH}/*/*.jpg")
