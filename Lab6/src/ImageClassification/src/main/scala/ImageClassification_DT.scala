...
object ImageClassification_DT {
  def main(args: Array[String]) {
    val IMAGE_CATEGORIES = Array("Dinosaur", "Dinosaur", "Dinosaur", "Dinosaur", "Coworker", "Coworker", "Coworker", "Coworker", "Pratt", "Pratt", "Pratt", "Pratt", "Pratt")
    ...
    val test = sc.textFile("data/test/testfeatures.txt")
    ...
    //TODO - This is where the fuzzy stuff is happening
    ...
    val y: RDD[(Double, Double)] = prediction1.map(f => {
      (floor(f._3.toDouble / 4.4),f._1)
    })

    println ("About to process the y.collect println")
  ...
  def testSpecificImage(imageName: String):String = {
    // I get it now.  These generate way too much logging
    Logger.getLogger("org").setLevel(Level.OFF);
    Logger.getLogger("akka").setLevel(Level.OFF);
    ...
    val sc = SparkContext.getOrCreate(sparkConf);
    val train = sc.textFile("data/train/features.txt")
    val test = sc.textFile("data/test/" + imageName + ".txt")
    ...
    val resArray = y.collect()
    println(resArray(0)._1)

    var result = "Decision Tree gave me the result of "
    if (resArray(0)._1 == 0.0)
      result = result + "Dinosaur"
    if (resArray(0)._1 == 1.0)
      result = result + "Coworker"
    if (resArray(0)._1 == 2.0)
      result = result + "Chris Pratt"
